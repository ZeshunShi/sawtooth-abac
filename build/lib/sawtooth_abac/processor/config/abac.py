...
def load_default_abac_config():
    """
    Returns the default abacConfig
    """
    return abacConfig(
...
def load_toml_abac_config(filename):
    """Returns a abacConfig created by loading a TOML file from the
    filesystem.
    ...
    Returns:
        config (abacConfig): The abacConfig created from the stored
            toml file.
...
def merge_abac_config(configs):
    """
    Given a list of abacConfig objects, merges them into a single
    abacConfig, giving priority in the order of the configs
    (first has highest priority).

    Args:
        config (list of abacConfigs): The list of abac configs that
            must be merged together

    Returns:
        config (abacConfig): One abacConfig that combines all of the
            passed in configs.
    """
    connect = None
